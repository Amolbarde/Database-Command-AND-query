...
CREATE TABLE employees (id INT,salary DECIMAL(10,2) CHECK (salary BETWEEN 30000 AND 150000));
CREATE TABLE   
...
CREATE TABLE orders (  id INT,status VARCHAR(20) CHECK (status IN ('PENDING', 'SHIPPED', 'DELIVERED', 'CANCELLED')));
CREATE TABLE
...
CREATE TABLE accounts (username VARCHAR(50)CHECK
(LENGTH(username) >= 5));
